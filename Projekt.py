        ...
        while not self.__networksuicide:
                    ...
                    for i in range(0, len(self.__ConfigData), 1):
                        if self.__ConfigData[i][0].find(self.__ConfigQueue[0][0]) > -1:
                            if self.__ConfigData[i][1] != self.__ConfigQueue[0][1]:
                                if self.debug:
                                    print
                                    "   Configuration missmatched, resending !"
                                    print
                                    "   " + self.__ConfigData[i][0] + " should be \"" + self.__ConfigQueue[0][
            ...
            if not len(self.__ConfigQueue):
                if not getconfigtag:
                    ...
                    time.sleep(sleeptime)
        if self.debug:    print
        "sendConfig-Tread :   committed suicide"
        ...
        while not self.__networksuicide:
            ...
            for ip in in_pipe:  # ...go and get it
                ...
                if ip == self.__vdecode_pipe:  ### Receiving imagedata and feedback from videodecode-process
                    ...
                    if self.showCommands and cmd != "Image":    print
                    "** vDec -> Com :", cmd
                ...
                if ip == self.__Video_pipe:  ### Receiving feedback from videostream-process
                    ...
                    if self.showCommands and cmd != "":    print
                    "** Vid -> Com : ", cmd
                ...
                if ip == self.__Config_pipe and not self.__networksuicide:  ### Receiving drone-configuration
                    try:
                        ...
                        if cfgdata.count("\x00"):  # Last byte of sent config-file, everything was received
                            ...
                            configdata, cfgdata = [], ""
                            if self.showCommands:    print
                            "Got " + str(len(self.__ConfigData)) + " Configdata " + str(time.time() - self.__calltime)
                            self.__calltime = 0
                    except IOError:
                        pass
                debug, showCommands = self.__checkAndReact(debug, showCommands)  # Check for errors and things to change
        if self.debug:    print
        "receiveData-Thread : committed suicide"
        # TestMe
        try:
...
def vCapture(VidPipePath, parent_pipe):
    ...
    capture.release()
    if debugV:    print
    "vCapture-Thread :    committed suicide"
...
def vDecode(VidPipePath, parent_pipe, parentPID):
    ...
    while not commitsuicideV:
        ...
        if cmd == "die!":
            ...
            debugV = True
            print
            "vDecode-Process :    running"
            if vCruns:    print
            "vCapture-Thread :    running"
        elif cmd == "undebug":
            debugV = False
    ...
    time.sleep(0.1)
    if debugV:    print
    "vDecode-Process :    committed suicide"


#####################################################
def VideoReceiveWatchdog(parent_pipe, name, debugV):
    if debugV:    print
    "WHATCHDOG reset von", name
    parent_pipe.send(("reset", 0, 0, 0))
...
def mainloopV(DroneIP, VideoPort, VidPipePath, parent_pipe, parentPID):
                    ...
                    if inited and not reset:
                        ...
                        if not saveVideo and lenVideoPackage > 45 and videoPackage[
                            ...
                            if videoPackage[30] == "\x01":  # I-Frame
                                ...
                                if debugV:
                                    print
                                    "*** Odd h264 Frametype: ", FrameCount,
                                    print
                                    videoPackage[30:50].encode("hex")
                                    print
                                    " - ", videoPackage[31:40].find("\x00\x00\x00"), ord(videoPackage[30])
                        elif not unsureMode:
                        ...
                        if not saveVideo and unsureMode and frameStart and lastIFrame and lenRawVideoFrame > 0:
                            ...
                            if burstFrameCount > 10:
                                ...
                                burstFrameCount = 0
                                if debugV: print
                                "To many pictures send while guessing the codec. Resetting."
    ...
    try:
        ...
        pass
    if debugV:    print
    "Video-Process :      committed suicide"
    try:
        vstream_pipe.close()
    except:
        pass
...
def decode_ID0(packet):  # NAVDATA_DEMO_TAG
    ...
    if dataset[1] != 148:        print
    "*** ERROR : Navdata-Demo-Options-Package (ID=0) has the wrong size !!!"
...
def decode_ID1(packet):  # NAVDATA_TIME_TAG
    ...
    if dataset[1] != 8:        print
    "*** ERROR : navdata-time-Options-Package (ID=1) has the wrong size !!!"
    time = [0.0]
...
def decode_ID2(packet):  # NAVDATA_RAW_MEASURES_TAG
    ...
    if dataset[1] != 52:        print
    "*** ERROR : navdata-raw_measures-Options-Package (ID=2) has the wrong size !!!"
...
def decode_ID3(packet):  # NAVDATA_PHYS_MEASURES_TAG
    ...
    if dataset[1] != 46:        print
    "*** ERROR : navdata-phys_measures-Options-Package (ID=3) has the wrong size !!!"
...
def decode_ID4(packet):  # NNAVDATA_GYROS_OFFSETS_TAG
    ...
    if dataset[1] != 16:        print
    "*** ERROR : navdata-gyros_offsets-Options-Package (ID=4) has the wrong size !!!"
    gyros_offsets = [0, 0, 0]
...
def decode_ID5(packet):  # NAVDATA_EULER_ANGLES_TAG
    ...
    if dataset[1] != 12:        print
    "*** ERROR : navdata-euler_angles-Options-Package (ID=5) has the wrong size !!!"
    euler_angles = [0, 0]
...
def decode_ID6(packet):  # NAVDATA_REFERENCES_TAG
    ...
    if dataset[1] != 88:        print
    "*** ERROR : navdata-references-Options-Package (ID=6) has the wrong size !!!"
...
def decode_ID7(packet):  # NAVDATA_TRIMS_TAG
    dataset = struct.unpack_from("HHfff", packet, 0)
    if dataset[1] != 16:        print
    "*** ERROR : navdata-trims-Options-Package (ID=7) has the wrong size !!!"
    trims = [0, 0, 0]
...
def decode_ID8(packet):  # NAVDATA_RC_REFERENCES_TAG
    ...
    if dataset[1] != 24:        print
    "*** ERROR : navdata-rc_references-Options-Package (ID=8) has the wrong size !!!"
...
def decode_ID9(packet):  # NAVDATA_PWM_TAG
    ...
    if dataset[1] != 76 and dataset[1] != 92:  # 92 since firmware 2.4.8 ?
        print
        "*** ERROR : navdata-navdata_pwm-Options-Package (ID=9) has the wrong size !!!"
...
def decode_ID10(packet):  # NAVDATA_ALTITUDE_TAG
    ...
    if dataset[1] != 56:        print
    "*** ERROR : navdata-navdata_altitude-Options-Package (ID=10) has the wrong size !!!"
...
def decode_ID11(packet):  # NAVDATA_VISION_RAW_TAG
    dataset = struct.unpack_from("HHfff", packet, 0)
    if dataset[1] != 16:    print
    "*** ERROR : navdata-vision_raw-Options-Package (ID=11) has the wrong size !!!"
    vision_raw = [0, 0, 0]
...
def decode_ID12(packet):  # NAVDATA_VISION_OF_TAG
    ...
    if dataset[1] != 44:    print
    "*** ERROR : navdata-vision_of-Options-Package (ID=12) has the wrong size !!!"
...
def decode_ID13(packet):  # NAVDATA_VISION_TAG
    ...
    if dataset[1] != 92:    print
    "*** ERROR : navdata-vision-Options-Package (ID=13) has the wrong size !!!"
...
def decode_ID14(packet):  # NAVDATA_VISION_PERF_TAG
    ...
    if dataset[1] != 108:        print
    "*** ERROR : navdata-vision_of-Options-Package (ID=14) has the wrong size !!!"
...
def decode_ID15(packet):  # NAVDATA_TRACKERS_SEND_TAG
    ...
    if dataset[1] != 364:        print
    "*** ERROR : navdata-trackers_send-Options-Package (ID=15) has the wrong size !!!"
...
def decode_ID16(packet):  # NAVDATA_VISION_DETECT_TAG
    dataset = struct.unpack_from("HHIIIIIIIIIIIIIIIIIIIIIIIIIffffIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIII",
                                 packet, offsetND)
    if dataset[1] != 328:    print
    "*** ERROR : navdata-vision_detect-Package (ID=16) has the wrong size !!!"
...
def decode_ID17(packet):  # NAVDATA_WATCHDOG_TAG
    ...
    if dataset[1] != 8:    print
    "*** ERROR : navdata-watchdog-Package (ID=17) has the wrong size !!!"
...
def decode_ID18(packet):  # NAVDATA_ADC_DATA_FRAME_TAG
    ...
    if dataset[1] != 40:    print
    "*** ERROR : navdata-adc_data_frame-Package (ID=18) has the wrong size !!!"
    adc_data_frame = [0,
...
def decode_ID19(packet):  # NAVDATA_VIDEO_STREAM_TAG
    ...
    if dataset[1] != 65:    print
    "*** ERROR : navdata-video_stream-Package (ID=19) has the wrong size !!!"
...
def decode_ID20(packet):  # NAVDATA_GAMES_TAG
    ...
    if dataset[1] != 12:    print
    "*** ERROR : navdata-games-Package (ID=20) has the wrong size !!!"
    games = [0, 0]
...
def decode_ID21(packet):  # NAVDATA_PRESSURE_RAW_TAG
    ...
    if dataset[1] != 18:    print
    "*** ERROR : navdata-pressure_raw-Package (ID=21) has the wrong size !!!"
...
def decode_ID22(packet):  # NAVDATA_MAGNETO_TAG
    ...
    if dataset[1] != 83:    print
    "*** ERROR : navdata-magneto-Package (ID=22) has the wrong size !!!"
...
def decode_ID23(packet):  # NAVDATA_WIND_TAG
    ...
    if dataset[1] != 56 and dataset[1] != 64:
        print
        "*** ERROR : navdata-wind_speed-Package (ID=23) has the wrong size !!!"
...
def decode_ID24(packet):  # NAVDATA_KALMAN_PRESSURE_TAG
    ...
    if dataset[1] != 72:    print
    "*** ERROR : navdata-wind_speed-Package (ID=24) has the wrong size !!!"
...
def decode_ID25(packet):  # NAVDATA_HDVIDEO-TAG
    ...
    if dataset[1] != 32:    print
    "*** ERROR : navdata-hdvideo_stream-Package (ID=25) has the wrong size !!!"
...
def decode_ID26(packet):  # NAVDATA_WIFI_TAG
    dataset = struct.unpack_from("HHI", packet, offsetND)
    if dataset[1] != 8:    print
    "*** ERROR : navdata-wifi-Package (ID=26) has the wrong size !!!"
...
def decode_ID27(packet):  # NAVDATA_ZIMU_3000_TAG
    ...
    if dataset[1] != 12 and dataset[1] != 216:  # 216 since firmware 2.4.8 ?
        print
        "*** ERROR : navdata-zimmu_3000-Package (ID=27) has the wrong size !!!"
    zimmu_3000 = [0, 0.0]
...
def decode_Footer(packet, allpacket):  ### Decode Checksum options-package ID=65535
    dataset = struct.unpack_from("HHI", packet, offsetND)
    if dataset[1] != 8:    print
    "*** ERROR : Checksum-Options-Package (ID=65535) has the wrong size !!!"
    chksum = [0, False]
    chksum[0] = dataset[2]
...
def mainloopND(DroneIP, NavDataPort, parent_pipe, parentPID):
    ...
    netHeartbeat.cancel()
    if debug:    print
    "NavData-Process :    committed suicide"
    # TestMe
    try:
        navdata_pipe.close()
    except:
...
if __name__ == "__main__":
    ...
    print
    "Batterie: " + str(drone.getBattery()[0]) + "%  " + str(drone.getBattery()[1])  # Gives a battery-status
